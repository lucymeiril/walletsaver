...
이마트 SSG 전단지 API를 통해 행사 상품 데이터를 수집한다.
SSG는 내부 API를 통해 전단지 데이터를 JSON으로 제공하므로
cloudscraper 전략으로 접근하여 파싱한다.

데이터 흐름: API JSON → DiscountItem → ProductPrice → DB
...
class EmartCrawler(CrawlerContract):
    """이마트 크롤러 — SSG 전단지/할인 행사 상품 수집."""

    BASE_URL = "https://emart.ssg.com"
    # 이마트 전단지 행사 상품 API (모바일 API가 더 안정적)
    API_URL = "https://emart.ssg.com/event/eventMain.ssg"
    SALE_API = "https://emart.ssg.com/disp/category.ssg"
    ...
    async def crawl(self) -> CrawlResult:
        ...
        try:
            ...
            response = requests.get(
                self.API_URL, headers=headers, timeout=20,
            )
            response.encoding = "utf-8"

            if response.status_code != 200:
                logger.error(f"[이마트] HTTP {response.status_code}")
                return CrawlResult(
