...
from services.review_publish import build_raw_ai_audit, proposals_by_raw_record
...
def _global_blockers(
    ...
    if not (raw_payload.get("image_url") or raw_payload.get("image")):
        blockers.append("raw record is missing image")
...
def _is_exact_category(proposal: FieldProposal, record: RawCrawlRecord, config: AutomationGateConfig) -> bool:
    if proposal.target_field != "category_id" and proposal.proposal_type != ProposalType.CATEGORY:
        return False
    ...
    if config.allowed_categories and expected not in set(config.allowed_categories):
        return False
    return str(proposal.proposed_value) == str(expected)
