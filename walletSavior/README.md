...
> ⚠️ **crawler-admin**, **db-admin**은 서버에서만 실행하는 내부 관리 도구입니다.
> 사용자에게 배포하지 않습니다.
...
┌─────────────┐    수집     ┌──────────┐    조회     ┌──────────────┐
│  크롤러 관리  │ ────────→ │    DB    │ ←──────── │  웹사이트 API  │
│ (crawler-admin)│          │(PostgreSQL)│           │   (FastAPI)   │
└─────────────┘            └──────────┘            └──────┬───────┘
                                │                         │
                           관리/조회                    데이터 전달
                                │                         │
...
1. **크롤러**가 정부 공공데이터·마트 세일·커뮤니티 핫딜을 수집
2. 수집된 데이터를 **DB에 저장** (PostgreSQL / 개발용 SQLite)
3. **웹사이트 API**가 DB에서 데이터를 읽어 가공
4. **웹사이트 UI**가 사용자에게 가격 비교·핫딜 정보를 표시
