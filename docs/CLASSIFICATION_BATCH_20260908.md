...
## 코스트코 치즈 진열 전량 감사·붙은 묶음 표기 수정 및 pass27 검증 완료
...
## 코스트코 과일 검토 및 pass28 — 최신 (2026-09-10)

- 미분류 69페이지를 전량 읽고 단일 과일이 확인된 23관측을 분류했다. 라임·자몽·용과 전용 리프를 추가했다. 혼합 과일, 분말차, 주스, 원액기와 근사중량 상품은 이번 분류에서 보류했다. 정확히 검토한 제목과 마트/진열이 일치할 때만 적용한다.
- 16관측 신규 포함, 수량 충돌 7관측 보류. 이전 4,111관측과 수동 결정 431건 모두 보존. 원본 9,196관측 전체 비교에서 기존 분류 변경은 0건이다.
- 상품군 2,614 / variant 2,644 / listing 2,764 / offer 4,127 / 매칭 규칙 2,691. 카테고리 317 / 키워드 242. 보류 5,069, 리프 미지정 4,857. active 3,388 / pending 739.
- 공개 API 4상품 중 라임·자몽은 총중량과 100g 가격을 확인했다. 용과·망고는 checkout_discount 조건 미확정으로 비교가격/단위가격이 없음을 확인했다. active offer 수를 비교 가능 수로 표현하지 않는다.
- DB 관리자 782 passed, 전체 accounting·재적용·snapshot·runtime 검사 통과. bundle SHA-256 `df3eb1e50e9b615b7c894d62ca2379e58b6a457b8f773fd42d8bb43f3efa6ef1`. 운영 DB는 변경하지 않았다.
# 2026-09-10 pass29 — 코스트코 고기 검색 진열 감사

- 최신 출력: `.debug-artifacts/initial-catalog-20260910-pass29`. 아래 이전 pass 기록은 이력이다.
- 미분류 74개 제목을 전량 읽었다. 원재료 고기 외에 조리식품·소스·토마토·미역·양갱, 그릴·숯·사료까지 섞여 있어 원본 검색 분류를 통합 리프로 재사용하지 않았다.
- 명시 제목 22개만 기존 리프 및 조리잡채/양념육/훈제오리/순대 리프에 배정했다. 조리잡채를 당면으로, 미역국을 미역으로, 스테이크소스를 신선육으로 넣지 않는다. 이름 변경·다른 진열·혼합세트에는 검토 결정을 확장하지 않는다.
- 15관측 신규 적재 / 규격 보류 7관측. 이전 포함 관측 전부 유지, 누적 수동 결정 431개 동일. 9,196관측 전체 분류 차이는 위 22건뿐이다. 검토 초안으로서 운영 승인과 별개다.
- 상품군 2,629 / variant 2,659 / listing 2,779 / offer 4,142 / 규칙 2,706. 보류 5,054 / 미지정 리프 4,835. 카테고리 320 / 키워드 245 / 매핑 237.
- 관리자 796 passed(기존 경고 460), taxonomy 집중 391 passed. 반복 import·원본 해시·snapshot·runtime 검증 통과. snapshot 보류 739개 제외/3,403개 유지. 이번에는 개별 상품 API 추가 검사는 실행하지 않았다.
- 다음 규격 조사: 천 단위 쉼표 중량과 연속 곱셈 묶음. 보류를 없애기 위한 임의 수량 입력은 하지 않았다.
# 2026-09-10 pass30 — 공통 규격 병목 해소

- 최신 `.debug-artifacts/initial-catalog-20260910-pass30`. 수집 원본·누적 결정 431개 유지.
- 천 단위 쉼표/단일 중량의 연속 곱셈 파서 수정, Costco 수집/출력 경로도 동일 파서 사용. 기존 수집 필드 충돌은 자동 덮어쓰지 않는다.
- 93관측 규격/진단 영향 전량 감사, 58개 단위 문제 해소, 34관측 추가 적재. 대량 연속 묶음 6개 검토 보류. 모든 기존 포함 관측 보존.
- 누적 offer 4,176 / unresolved 5,020 / leaf missing 4,835. 상품군 2,663 / variant 2,693 / listing 2,813 / 규칙 2,740 / 카테고리 323 / 키워드 248 / 매핑 237.
- 관리자 802/shared 161/Costco 14 passed. 구식 연속곱셈 무조건 보류 계약은 완전 해석·필드 일치 시 허용하는 계약으로 갱신하고 충돌/부분해석/혼합/대량 묶음 회귀 검사 유지. 단순 통과 목적의 검사 삭제 아님.
- 독립 stage·snapshot·runtime 검증 및 실제 API 3상품 총량/단위가 검사 통과. 운영 DB 수정/공개 없음. 전체 4,835 미분류를 해결한 것은 아니며 다음에는 여러 카테고리를 묶어 검토한다.
# 2026-09-10 pass31 — 이마트 간편식·면류·베이커리 일괄 감사

- 최신 출력 `.debug-artifacts/initial-catalog-20260910-pass31`, 누적 결정 입력 기존 431건. 대상 미분류 327관측 전량 제목 검토, 148개 정확한 상품명에 분류 제안. 식빵/베이글/생지/잼 등 11개 세부 리프 추가.
- 전체 원본 비교 245관측 분류 변화, 모두 대상 3경로의 미분류→리프. 최종 workspace coverage +235 / 포함 +216. 원본 listing의 이름 변경·규격 검사와 누적 결정을 적용하므로 단건 분류기 제안 수와 최종 적재 수는 다르다. 이전 포함 전량 유지.
- 상품군 2,792 / variant 2,822 / listing 2,942 / offer 4,392 / 규칙 2,869. 보류 4,804 / 미분류 4,600. 카테고리 337 / 키워드 260 / 매핑 237.
- 전체 관리자 960 passed, 독립 stage/snapshot/runtime/API 4상품 검사 통과. 우주인피자는 제목 변경 충돌로 보류되었음을 따로 확인, 검사를 통과시키려고 적재하지 않았다. 발표용 공개 데이터나 운영 DB는 변경하지 않았다.
# 2026-09-10 pass32 — 이마트 생활·위생·뷰티 3묶음

- 최신 `.debug-artifacts/initial-catalog-20260910-pass32`. 191개 미분류 제목 전량 검토, 145개 명시 분류 추가. 전체 원본에서 대상 외 분류 변화 없음. 최종 coverage +143, 검토 DB +134.
- 캡슐/액체/건조기시트, 염색제/샴푸, 과탄산 표백제/락스, 청소용티슈/일반물티슈를 분리했다. 원본 제목이 모호한 위생제품과 혼합 선물세트는 억지 배정하지 않았다.
- 누적 상품군 2,926 / variant 2,956 / listing 3,076 / offer 4,526 / 규칙 3,003. 보류 4,670 / 미분류 4,457. 카테고리 369 / 키워드 288 / 매핑 237.
- 관리자 1,113 passed, 집중 544 passed. 기존 포함 전량/431결정 보존, 반복 적재·stage·snapshot·runtime·실제 API 4상품 리프/총량/단위가 검사 통과. 원본 운영 DB 및 공개 서비스는 변경하지 않았다.
# 2026-09-10 pass33 — 홈플러스 상세 경로 4묶음

- 소시지/가공 닭가슴살/반찬/곡물스낵 전량 제목 검토 후 경로+제품 형태 규칙 추가. 149관측/76제목 분류 변화, 기존 분류 변경 없음. 최종 145관측 추가 적재, 기존 포함과 431개 수동 결정 유지.
- 상품군 3,000 / variant 3,030 / listing 3,150 / offer 4,671 / 규칙 3,077. 보류 4,525 / 미분류 4,312. 카테고리 373 / 키워드 292 / 매핑 237. 새 적재 중 프로모션 불명확 4관측은 공개 제외.
- 관리자 1,122 passed. 멱등 적재·stage·snapshot·runtime 검증 통과, 개별 API 추가 검사는 미실행. 원본/운영 공개 변경 없음. 메추리알 장조림의 기존 이름 근거 충돌은 검사 완화 없이 보류.
# 2026-09-10 pass34 — 홈플러스 소스·유부초밥재료·젤리

- 전체 비교 101관측/52제목 분류 추가, 최종 95관측 적재. 유부초밥재료를 완성 초밥으로 분류하지 않음. 순두부찌개 양념은 이미 양념으로 거부되는 재료명 후보를 소스 근거가 확실할 때만 제외하여 분류.
- 상품군 3,049 / variant 3,079 / listing 3,199 / offer 4,766 / 규칙 3,126. 카테고리379 / 키워드297 / 매핑238. 보류4,430 / 미분류4,217.
- 관리자1,130 passed. 실제 DB 매칭규칙→상품군/규격→카테고리 및 키워드→카테고리 조인 확인. 기존 포함/431결정 보존, stage/snapshot/runtime 검사 통과. 운영 적용 및 실재수집은 아직 미완료.
# 2026-09-10 pass35 — 이마트 정육·수산·반찬

- 3개 경로 미분류 제목 전량 검토, 최종 116관측 분류 변화/109관측 신규 적재. 원물과 조리식품을 구분하고 완성초밥/김밥/샐러드 및 생선종·김치종 리프 추가. 초기 감사의 행사 모음 2건 후보는 퍼센트 표기 제외와 회귀 검사로 제거.
- 누적 상품군3,158 / variant3,188 / listing3,308 / offer4,875 / 규칙3,235. 보류4,321 / 미분류4,104. 카테고리392 / 키워드310 / 매핑238.
- 관리자1,143 passed, 기존 포함/431결정 보존, stage/snapshot/runtime 검증 성공. 개별 API 추가 검사는 미실행. 운영 DB/공개 수정 없음.
# pass36 (2026-09-10): 홈플러스 냉장 음료·디저트

냉장주스·신선음료·푸딩디저트류에서 실제 상품 형태로 분류했다. 68관측/36제목 제안 변화, 최종64관측 추가. 신규 리프는 귀리음료·식혜·푸딩(전통음료 부모 포함 카테고리+4). 기존 분류/포함/431수동결정 보존. 누적 검토 적재4,939, 보류4,257이며 운영 승인본은 아니다.

관리자1,160통과, 멱등 import/stage/snapshot/runtime 및 실제 API4상품 검증. 야채사랑365 190ml×4 1+1은 총1,520ml/7,590원/100ml499원: 한 팩만 예상했던 검사식을 실제 행사 계약으로 수정했고 계산 코드는 그대로다. 원본 SHA 불변, 생성 DB는 Git 제외. 최신 출력과 다음 작업은 RESUME_CHECKPOINT.md pass36 참조.
