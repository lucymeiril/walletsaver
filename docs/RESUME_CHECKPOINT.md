...
할당량 중단 후 재개한 작업 기록이다. 최신은 pass22이며 상세 근거는 `CLASSIFICATION_BATCH_20260908.md` 마지막 절을 우선한다. 다음 작업은 이 문서와 `git status`를 함께 확인한다. 아래 초기 카탈로그는 **별도 검증 DB의 초안이며 운영 승인본이 아니다**. pass4 등 과거 수치는 이력으로 보존한다.
...
- 사용자에게 설명한 완료 경계: 원본 백업과 별도 검토용 DB 구축은 했지만, 운영 관리자 DB의 초기 매칭·카테고리·키워드·상품 테이블을 모두 완성하여 적용한 것은 아니다. 최신 pass22도 검토용 초안이다. 남은 상품 분류를 완료한 뒤 기존 승인 절차로 운영 적용해야 한다.
...
2026-09-09 최신은 **pass22/커피 카테고리 전량 감사/누적 431개 수동 결정 유지**다. 상품군 2,339 / variant 2,369 / listing 2,489 / offer 3,850, 보류 5,346관측이다. 최신 결과는 `docs/CLASSIFICATION_BATCH_20260908.md` 마지막 절을 우선한다. 위 pass4 건수는 이전 기록이다. 조사·제안 파일을 승인본으로 취급하지 않는다.

1. 최신 전체 테스트 결과와 `git status`를 확인한다. 아래 완료한 단위/스냅샷 수정을 다시 시작하지 않는다.
2. 현재 pass22가 최신이다. 분류 코드/검토 문서 변경 후에는 새 출력 폴더에 workspace를 재생성한다. 누적 431개 결정을 유지하려면 아래 `--review-decisions`를 반드시 사용한다.
3. 5,346개 보류 중 5,225개가 리프 assignment 미지정이다. 커피 카테고리 방식처럼 다음 원본 카테고리도 전체 이름과 오염 예외를 한 번에 검토한다. 커피 진열의 차·용품은 커피로 흡수하지 않았으므로 관련 카테고리 감사에서 다시 판단한다.
...
- DB 관리자 전체: **633 passed**, 460 existing warnings, 43.94s (2026-09-09).
