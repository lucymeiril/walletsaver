...
할당량 중단 후 재개한 작업 기록이다. 최신은 pass9이며 상세 근거는 `CLASSIFICATION_BATCH_20260908.md`를 우선한다. 다음 작업은 이 문서와 `git status`를 함께 확인한다. 아래 초기 카탈로그는 **별도 검증 DB의 초안이며 운영 승인본이 아니다**. pass4 등 과거 수치는 이력으로 보존한다.
...
2026-09-08 최신은 **pass9/누적 238개 결정/39개 수동 병합군 검증 완료**다. 상품군 2,295 / variant 2,299 / listing 2,339 / offer 3,654, 보류 5,542관측이다. 최신 결과는 `docs/CLASSIFICATION_BATCH_20260908.md`를 우선한다. 위 pass4 건수는 이전 기록이다. 조사·제안 파일을 승인본으로 취급하지 않는다.

1. 최신 전체 테스트 결과와 `git status`를 확인한다. 아래 완료한 단위/스냅샷 수정을 다시 시작하지 않는다.
2. 현재 pass9가 최신이다. 분류 코드/검토 문서 변경 후에는 새 출력 폴더에 workspace를 재생성한다. 누적 238개 결정을 유지하려면 아래 `--review-decisions`를 반드시 사용한다.
3. 다음 좁은 원본 범위를 전량 검토하고 분류를 보강한다. 5,542개 보류 중 5,434개는 리프 assignment가 아직 없다. 신선식품 batch2/batch4와 곡물·컵라면 9군의 완료 항목을 중복 작업하지 않는다. 소스/조미료 또는 다른 미검토 가공식품 상품군이 다음 후보이다.
