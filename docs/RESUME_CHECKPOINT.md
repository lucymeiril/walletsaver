...
- 기준 `.debug-artifacts/initial-catalog-20260914-pass49-final/checks-passed.json`. 6,014적재 / 3,182미해결. 홈플러스 레몬즙480ML(070812407) / 레몬 티앤에이드680G(070414556), 관측4개 추가. 상품군/규격/판매페이지/매칭규칙 각2개, 리프/키워드 각2개 추가.
- 관련810검사·DB·멱등import·매칭119상품군/375hit·25miss/경로당1,200변형 통과。 원본/복원DB 해시·기존 포함행·431결정 보존, 공개승인 없음. certificate는 기준 승격 전 state를 기록한다.
- 레몬즙14T는 내용량 미확인으로 보류. 초안pass49는14T도 적재했으므로 DO_NOT_USE.md로 사용금지 표시, 삭제하지 않음. 최종은 T표기 제외 회귀검사 포함. 레몬즙은 정확한 홈플러스 농축액 경로만 사용하며 건강식품/넓은 커피 경로로 확대하지 않았다.
- 다음은 현재 unresolved의 주방용품 묶음(비코스트코42관측)을 원문 전체 검토해 공통 상품형태로 처리. 기존101/103/104 완료분 반복 금지. 코스트코 계란 경로62개에는 고기/팬 등이 섞여 있으므로 경로 이름만으로 일괄 처리하지 않는다.
