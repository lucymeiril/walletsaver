...
- 대형마트 할인 정보 수집 + 마트4사+쿠팡 자체 수집 기준가와 비교
- 핫딜 게시판 통합 크롤링
- 배달앱/식당 가격 비교
- 주유소 가격 비교
- 물가 변동 추이 제공
...
| 데이터 소스 | 신뢰도 | 용도 |
|---|---|---|
| **마트4사+쿠팡 수집가** | ★★★ 최고 | 식료품 기준 가격 (baseline) |
| **마트 전단/정가** | ★★★ 높음 | 일반 시장가 + 할인가 DB |
| **핫딜 게시판** | ★☆☆ 낮음 (표시용) | 최저가 참고만, 평균 산출에 불포함 |
| **배달앱 가격** | ★★ 중간 | 외식 가격 참고 |

**DB 분리 설계:**
- `baseline_prices` — 마트4사+쿠팡 수집가로만 구성. **오염 불가능한 기준 테이블**
...
### 5.1 공공 API (2개 + 폐기 1개) — API 키 필요
| 대상 | API | 수집 데이터 | API 키 상태 |
|---|---|---|---|
| **KAMIS** | kamis.or.kr | 식료품 가격 티어 경로에서는 폐기됨. 레거시 문서용 보존 항목 | 🚫 사용 중단 |
...
| `products` | 품목 마스터 |
| `official_prices` | 레거시 공공 가격 아카이브 (신규 식료품 기준가에는 미사용) |
...
| Phase | 내용 | 우선순위 |
|---|---|---|
| 3 | 공공 API 크롤러 (OPINET, KOSIS 중심) | API 키 발급 후 |
| 4 | 대형마트 크롤러 (이마트, 홈플러스, 롯데마트, 코스트코, 쿠팡) | 높음 |
...
- [ ] data.go.kr에서 OPINET, KOSIS API 키 발급
