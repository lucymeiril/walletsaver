...
- `pending 001~037`: **1,735 observations**. 이 구간은 현재 역순 strict audit가 아직 끝나지 않았다.
- pass41 전체 pending 3,916 대비 1,735는 약 **44.3%**다.
- 따라서 관측 구간만 놓고 보면 `038~451`은 전체 pending의 약 **55.7%**에 해당한다. 다만 이것을 곧바로 “55.7% 최종 완료”라고 부르면 안 된다. 과거 proposal 중 중복/re-review가 있고 taxonomy hold도 남아 있기 때문이다.
- `001~037` 안에도 과거 proposal이 존재하는 그룹(예: 028, 034)이 있으므로, 실제 새 판단량은 1,735보다 작을 가능성이 높다. 그러나 raw-key 단위 전역 reconciliation 전에는 더 작은 숫자를 완료치로 확정하지 않는다.
...
### reverse sweep 038~043 — 완료
통합 체크포인트: `checkpoints/checkpoint-reverse-sweep-038-043.md`

- observations opened: **148**
- already classification-complete exclusions: **5** (pending 040)
- new classification-review observations: **143**
- distinct newly reviewed source listings: **131**
- confirmed existing-leaf proposals: **42 listings**
- taxonomy/product-form/manual-review holds: **89 listings**
...
## 4. 현재 재개점 — pending 037

**다음 AI는 새 그룹을 고르지 말고 여기서 이어간다.**

- group: `pending/037`
- shelf: emart `생수/음료/주류`
- PENDING_INDEX count: **27 observations / 27 titles**
- files: `pending/037/001.json`, `pending/037/002.json` — 두 파일 모두 확인해야 한다.
- 현재 검토 시작됨. 001/002에 이미 `review_status=classified`인 음료가 섞여 있고, 일부만 classification-pending이다. 따라서 27 전체를 신규 분류량으로 세지 말 것.
- 확인된 예: `처음 보리차`, `아이얌 ... 프룬사과 주스`, `아이얌 ... 배도라지 주스`, `해태 갈아만든배` 등은 이미 분류되어 단위 문제 등으로 pending에 남아 있다.
- classification-pending 예: `오늘 사과해 100ml`, `오트몬드 프로틴 쿠키앤크림 250ml`, `따옴 오렌지 730ml` 등.
...
4. existing pass41 drink leaf를 먼저 재사용하고, 없는 제품형태만 hold/new taxonomy candidate로 남긴다.
5. `proposals/emart-beverages-037.json` 저장.
6. 이 `CURRENT_STATUS.md`의 숫자/재개점을 `036`으로 갱신하고 체크포인트를 남긴다.

## 5. 문서 신뢰 우선순위

다음 AI가 최초 사용자 프롬프트만 받은 경우 아래 순서로 읽는다.

1. `handoff/2026-09-11/README.md` — 작업 계약
