...
4. 사용자 보고 마지막에는 저장소 자료로 계산한 **DB 반영 후보로 작업 완료한 누적 관측 수 / 아직 남은 관측 수 / 전체 대비 진척률**을 반드시 붙인다. 여기서 완료 관측은 실제 DB import 완료만 뜻하지 않는다. 원본 identity 대조와 의미 검토가 끝나 현재 proposal이 `existing_leaf` 또는 기존 분류를 보존하는 `already_classified`처럼 **통합 담당자가 DB 반영 검증에 바로 넘길 수 있는 상태**인 관측도 완료에 포함한다. `hold`, 신규 리프 미승인, 형식 오류, identity 충돌, 의미 미검토 등 현재 상태로 바로 반영 후보가 아닌 관측은 남은 관측으로 센다. 진척률은 `완료 관측 / (완료 관측 + 남은 관측) * 100`으로 계산한다. 같은 `raw_record_id`가 여러 제안에 중복되면 한 번만 센다. 실제 DB 반영 완료 수치와 proposal-ready 진척도는 서로 다른 지표이므로 혼동하지 말고 필요하면 둘 다 구분해 쓴다. 수치는 스크립트·DB 질의·구조화된 저장소 자료처럼 재현 가능한 계산으로 얻고, 손으로 누적하지 않는다. 계산 근거가 부족하면 추측값 대신 마지막 재현 가능한 제안 집계와 그 기준 commit을 명시한다.
