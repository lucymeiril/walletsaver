...
4. 사용자 보고 마지막에는 서로 다른 두 진척도를 함께 붙인다. **검토 진척도**는 identity/의미 판단이 끝난 고유 `raw_record_id`를 decision 종류와 관계없이 완료로 세어 `검토 완료 / (검토 완료 + 미검토)`로 계산한다. 이 값은 실제 분류 작업량이 얼마나 진행됐는지 보여준다. **DB-ready 진척도**는 그중 현재 proposal이 `existing_leaf` 또는 기존 분류를 보존하는 `already_classified`처럼 통합 담당자가 기존 taxonomy로 바로 DB 반영 검증에 넘길 수 있는 고유 관측만 완료로 세고, `hold`, `new_leaf_needed`, 신규 리프 미승인, identity 충돌, 형식 오류는 남은 것으로 센다. 실제 DB import 완료 수와 DB-ready proposal 수는 별도 지표이며 혼동하지 않는다. 같은 `raw_record_id`는 여러 제안에 있어도 한 번만 센다. 수치는 스크립트·DB 질의·구조화된 저장소 자료처럼 재현 가능한 근거로 계산하고 손으로 누적하지 않는다. 전체 DB-ready 누계를 안전하게 재계산할 근거가 부족하면 마지막 검증 가능한 DB-ready 기준값과, 이번 작업에서 새로 증가한 고유 관측 수를 따로 명시한다.
...
- 중복 제안 대신 기존 파일 보완+수정 이유를 남긴다. 제안에는 상품별 근거를, 상태에는 파일 링크와 다음 지점만 쓴다. 누적 건수·완료율은 반드시 재현 가능한 계산으로 산출하며 손으로 더하지 않는다. ‘제안 완료’와 ‘DB 반영 완료’를 구분한다.
