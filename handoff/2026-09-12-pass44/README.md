...
4. 사용자 보고 마지막에는 저장소 자료로 계산한 **DB 반영 후보로 작업 완료한 누적 관측 수 / 아직 남은 관측 수 / 전체 대비 진척률**을 반드시 붙인다. 진척률은 `완료 관측 / (완료 관측 + 남은 관측) * 100`으로 계산한다. `hold`, 형식 오류, identity 충돌 등 현재 상태로 DB 반영 후보가 아닌 관측은 완료 관측에 넣지 않는다. 같은 관측 ID가 여러 제안에 중복되면 한 번만 센다. 수치는 스크립트·DB 질의·구조화된 저장소 자료처럼 재현 가능한 계산으로 얻고, 손으로 누적하지 않는다. 계산 근거가 부족하면 추측값 대신 마지막 검증 수치와 그 기준 commit을 명시한다.
