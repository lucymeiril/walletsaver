...
def _category_display_label(category_id: str | None, category_name: str | None = None) -> str:
    return category_name or SEED_CATEGORY_DISPLAY_LABELS.get(category_id or "", category_id or "")
...
def _enrich_product(session: Session, p: Product) -> dict:
    ...
    try:
        ...
        return {
            ...
            "category_display_label": _category_display_label(p.category_id, cat_name),
            "unit": p.unit,
            ...
            "external_seller": bool(p.external_seller),
            "unit_price_displayed": p.unit_price_displayed,
            "unit_price_display": (p.attributes or {}).get("unit_price_display") or ((latest.raw_data or {}).get("unit_price_display") if latest else None),
            "unit_price_basis_raw": p.unit_price_basis_raw,
            "mart_native_category_id": p.mart_native_category_id,
            "mart_native_category_path": p.mart_native_category_path,
            "canonical_url": p.canonical_url,
            "mart_internal_seller_id": p.mart_internal_seller_id,
        ...
        return {
            ...
            "mart": getattr(p, "mart", None),
            "mart_native_code": getattr(p, "mart_native_code", None),
            "canon_hash": getattr(p, "canon_hash", None),
            "external_seller": bool(getattr(p, "external_seller", False)),
            "unit_price_displayed": getattr(p, "unit_price_displayed", None),
            "unit_price_display": (getattr(p, "attributes", None) or {}).get("unit_price_display"),
            "unit_price_basis_raw": getattr(p, "unit_price_basis_raw", None),
            "mart_native_category_id": getattr(p, "mart_native_category_id", None),
            "mart_native_category_path": getattr(p, "mart_native_category_path", None),
            "canonical_url": getattr(p, "canonical_url", None),
            "mart_internal_seller_id": getattr(p, "mart_internal_seller_id", None),
            "promo_label": getattr(p, "promo_label", None),
            "promo_type": getattr(p, "promo_type", None),
    ...
    try:
        ...
        if sort_by in ("price", "discount_rate"):
            ...
            price_sub = (
                ...
                .filter(DiscountHistory.price > 0)
                .subquery()
            )
