...
def restore_sqlite_backup(backup_path: str, db_path: str) -> str:
    """
    Restore a SQLite database from a verified backup file.

    The restore is explicit and local-only; callers should create a fresh
    pre-restore backup separately when preserving the current broken state is
    required.
    """
    ...
    try:
        src.backup(dst)
        logger.warning("SQLite backup restored: %s -> %s", backup, target)
    finally:
        dst.close()
        src.close()
    return str(target)
