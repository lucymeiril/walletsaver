"""이마트 플러그인 adapter — CrawlerPlugin Protocol을 구현하는 얇은 어댑터."""
...
class EmartPlugin:
    ...
    async def crawl(self, targets: list[str] | None = None) -> RawBatch:
        ...
        try:
            ...
            logger.warning("[EmartPlugin] import failed: %s", exc)
            return RawBatch(plugin_name="emart", errors=[f"import_failed: {exc}"])
