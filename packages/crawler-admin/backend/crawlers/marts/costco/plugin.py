"""코스트코 플러그인 adapter — CrawlerPlugin Protocol 구현."""
...
class CostcoPlugin:
    ...
    async def crawl(self, targets: list[str] | None = None) -> RawBatch:
        ...
        try:
            ...
            logger.warning("[CostcoPlugin] import failed: %s", exc)
            return RawBatch(plugin_name="costco", errors=[f"import_failed: {exc}"])
