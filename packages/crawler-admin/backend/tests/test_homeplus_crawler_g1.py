...
def test_dynamic_scroll_requires_two_of_three_stop_conditions():
    signals = HomeplusCrawler._homeplus_scroll_stop_signals(
        unchanged_streak=5, end_marker_present=True, latest_xhr_empty=False
    )
    assert signals["stop"] is True
    assert signals["satisfied"] == 2

    signals = HomeplusCrawler._homeplus_scroll_stop_signals(
        unchanged_streak=5, end_marker_present=False, latest_xhr_empty=True
    )
    assert signals["stop"] is True
    assert signals["satisfied"] == 2

    signals = HomeplusCrawler._homeplus_scroll_stop_signals(
        unchanged_streak=0, end_marker_present=True, latest_xhr_empty=True
    )
    assert signals["stop"] is True
    assert signals["satisfied"] == 2

    one_condition = HomeplusCrawler._homeplus_scroll_stop_signals(
        unchanged_streak=5, end_marker_present=False, latest_xhr_empty=False
    )
    assert one_condition["stop"] is False


def test_homeplus_source_requests_include_hyper_and_exp_category_lists():
    crawler = HomeplusCrawler()
...
def test_category_tree_fixture_is_available_for_g2():
    path = FIXTURES / "homeplus_category_tree_g1.json"
    data = json.loads(path.read_text(encoding="utf-8-sig"))

    assert len(data) == 54
    assert {row["storeType"] for row in data} == {"HYPER", "EXP"}
    assert all(row["url"].startswith("https://mfront.homeplus.co.kr") for row in data)
