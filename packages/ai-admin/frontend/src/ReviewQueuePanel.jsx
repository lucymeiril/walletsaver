...
const STATUS_COPY = {
  ...
  pending_db_review: { label: 'DB 검수 대기', help: '이미 DB-admin 검수 큐에 제출됐습니다. 중복 발행하지 말고 DB-admin 승인 또는 롤백을 진행하세요.' },
  published: { label: '최종 승인됨', help: 'DB-admin ai-safe-final-approve까지 성공했습니다. 문제 발견 시 롤백/재검수를 진행하세요.' },
...
function statusBadgeClass(status) {
  ...
  if (status === 'ai_proposed' || status === 'human_reviewing' || status === 'pending_review' || status === 'publishing' || status === 'pending_db_review') return 'warn';
...
function formatBlockerReason(reason) {
  ...
  if (reason.startsWith('pending_db_review:')) return 'DB-admin 검수 대기';
...
function categoryForProposal(proposal) {
  if (proposal.proposal_type === 'category') return formatCategoryDisplay(proposal.proposed_value);
  if (proposal.target_field === 'category_id') return formatCategoryDisplay(proposal.proposed_value);
...
function FirstScreenOperations({ publishSummary, providerSummary, healthCards, triageCounters }) {
  ...
  return (
    <div className="card workflow-card ops-command-center">
      ...
      <div className="batch-health-grid" aria-label="AI ops triage counts" style={{ marginTop: 10 }}>
        {triageCounters.map((card) => (
          <div key={card.key} className={`batch-health-card ${card.tone ? `health-${card.tone}` : ''}`}>
            <div className="pipeline-count">{card.value}</div>
            <strong>{card.label}</strong>
            <div className="muted">{card.help}</div>
...
function OperatorDashboardSummary({ operatorSummary, operatorSummaryError }) {
  if (operatorSummaryError) {
    return (
      <div className="card workflow-card">
        <span className="badge err">운영 요약 불러오기 실패</span>{' '}
        <span className="muted">{operatorSummaryError}</span>
      </div>
    );
  }
  if (!operatorSummary) {
    return (
      <div className="card workflow-card">
        <span className="badge warn">운영 요약 로딩</span>{' '}
        <span className="muted">차단/이상/검수 큐 요약을 불러오는 중입니다.</span>
      </div>
    );
  }

  const report = buildOperatorDashboardReport(operatorSummary);
  return (
    <div className="card workflow-card">
      <div className="row" style={{ justifyContent: 'space-between' }}>
        <strong>One-shot 운영 품질 요약</strong>
        <span className={`badge ${operatorSummary.read_only ? 'ok' : 'warn'}`}>
          {operatorSummary.read_only ? 'read-only' : '상태 확인 필요'}
        </span>
      </div>
      <div className="muted" style={{ marginTop: 6 }}>
        배치 {operatorSummary.batch_id || '전체'} · 차단 사유, anomaly bucket, DB-admin 검수 대기를 한 화면에서 확인합니다.
      </div>
      <div className="batch-health-grid" aria-label="one-shot operator dashboard counts">
      ...
      {(!!report.topBlockers.length || !!report.topAnomalies.length) && (
        <div className="split-view" style={{ marginTop: 10 }}>
          <div className="decision-hint">
            <strong>상위 발행 차단 행</strong>
            {report.topBlockers.length === 0 && <div className="muted">차단 행이 없습니다.</div>}
            <ul className="compact-list">
              {report.topBlockers.map((row) => (
                <li key={row.raw_record_id}>
                  <code>{row.raw_record_id}</code>{' '}
                  <span className={`badge ${row.eligible ? 'ok' : 'err'}`}>{labelStatus(row.status)}</span>
                  <div className="muted">{(row.blockers || []).slice(0, 3).map(formatBlockerReason).join(', ') || '차단 사유 없음'}</div>
                </li>
              ))}
            </ul>
          </div>
          <div className="decision-hint">
            <strong>상위 anomaly bucket</strong>
            {report.topAnomalies.length === 0 && <div className="muted">의심 버킷이 없습니다.</div>}
            <ul className="compact-list">
              {report.topAnomalies.map((bucket) => (
                <li key={bucket.code}>
                  <span className="badge err">{bucket.count || bucket.rows?.length || 0}건</span>{' '}
                  {bucket.message || labelIssue(bucket.code)}
                  <div className="muted"><code>{bucket.code}</code></div>
                </li>
              ))}
            </ul>
          </div>
  ...
  return (
    <div className="card workflow-card">
      ...
      <div className="muted">
        ...
        핵심 검증을 모두 통과한 행은 이 클릭으로 DB-admin 안전 최종 승인까지 요청하고, 감사 플래그가 남은 행은 DB-admin 검수 큐로 보냅니다.
      </div>
      <div className="decision-hint" style={{ marginTop: 8 }}>
      ...
      <div className="decision-hint" style={{ marginTop: 8 }}>
        ...
        <br />
        전체 eligible {totalEligible}개(최종 승인 후보 {publishSummary?.ai_safe_final_approve_count ?? 0}개, DB 검수 큐 {publishSummary?.db_review_handoff_count ?? 0}개) 중 현재 필터 eligible {count}개 · 보류/차단 {blockedCount}개 ·
      ...
      {!!approvedPreviewRows.length && (
        <details className="inline-details" style={{ marginTop: 8 }} open>
          <summary>승인/발행 가능 원본 {approvedPreviewRows.length}개 미리보기</summary>
          <ul className="compact-list">
            {approvedPreviewRows.map((row) => (
              <li key={row.raw_record_id}>
          ...
          <div className="muted">제출만으로 공개 DB 저장이 아닙니다. DB save complete는 DB-admin 최종 승인 성공 후입니다.</div>
        </details>
      )}
      ...
      {!!publishedRows.length && (
        <details className="inline-details" style={{ marginTop: 8 }} open>
          <summary>DB-admin 전송 완료/검수 대기·롤백 가능 원본 {publishedRows.length}개</summary>
          <ul className="compact-list">
    ...
    <li className="proposal-card">
      <div className="proposal-main">
        ...
        <div style={{ marginTop: 6 }}>
          <b>{labelField(proposal.target_field)}</b> = <code>{
            proposal.target_field === 'category_id' || proposal.proposal_type === 'category'
              ? formatCategoryDisplay(proposal.proposed_value)
...
function MatchDecisionCards({ cards, reviewerId, setReviewerId, onRefresh, onError }) {
  async function runAction(card, action, candidate) {
    if (!reviewerId.trim()) {
      onError('매치 카드: 검수자 ID를 입력하세요.');
      return;
    }
    const auditReason = window.prompt('감사 사유를 입력하세요 (필수)', `${action} by AI-admin operator`);
    if (!auditReason || !auditReason.trim()) return;
    const titlePattern = window.prompt('제목 패턴(비우면 원본 제목 사용)', card.raw_record.raw_title) || undefined;
    try {
      const body = {
        action,
        reviewer_id: reviewerId.trim(),
        audit_reason: auditReason.trim(),
        ...(candidate?.match_id ? { target_match_id: candidate.match_id } : {}),
        ...(action === 'add_allowed_title_pattern' ? { allowed_title_pattern: titlePattern } : {}),
        ...(action === 'add_blocked_title_pattern' ? { blocked_title_pattern: titlePattern } : {}),
        ...(action === 'create_variant_candidate' ? {
          canonical_product_name: card.raw_record.raw_title,
          target_type: 'variant',
        } : {}),
        ...(action === 'create_source_listing_candidate' ? {
          canonical_product_name: card.raw_record.raw_title,
          target_type: 'source_listing',
        } : {}),
      };
      await requestJson(`/api/review/match-cards/${encodeURIComponent(card.raw_record.raw_record_id)}/actions`, {
        method: 'POST',
        body,
      });
      onError(`매치 카드 처리 완료: ${action}`);
      await onRefresh();
    } catch (err) {
      onError(`매치 카드: ${err.message}`);
    }
  }
  ...
  return (
    <details className="inline-details" style={{ margin: '12px 0' }} open>
      <summary>DB 매치 결정 카드 ({cards.length}개 원본)</summary>
      <div className="muted" style={{ marginTop: 6 }}>
        원본 행과 후보 매치를 운영자용 카드로 보여줍니다. 상세 JSON은 각 카드의 고급 섹션에만 표시됩니다.
      </div>
      <label style={{ display: 'block', marginTop: 8 }}>
        검수자 ID
        <input value={reviewerId} onChange={(e) => setReviewerId(e.target.value)} placeholder="operator-id" style={{ marginLeft: 8 }} />
      </label>
      <div className="match-card-list">
        {cards.slice(0, 20).map((card) => (
          <div className="card match-card" key={card.raw_record.raw_record_id}>
            <div className="row" style={{ justifyContent: 'space-between' }}>
              <div>
                <strong>{card.raw_record.raw_title}</strong>
                <div className="muted">
                  {card.raw_record.source_name} · {card.raw_record.raw_record_id} · 가격 {pretty(card.raw_record.raw_price)}
                </div>
                <div className="muted">
                  패키지 {pretty(card.raw_record.package_signature)} · source product {pretty(card.raw_record.source_product_id)}
                </div>
              </div>
              <div className="row">
                <button onClick={() => runAction(card, 'create_variant_candidate')}>variant 생성</button>
                <button onClick={() => runAction(card, 'create_source_listing_candidate')}>source listing 생성</button>
            ...
            <div className="candidate-list">
              {(card.candidates || []).slice(0, 5).map((candidate) => (
                <div className="candidate-card" key={candidate.match_id || `${candidate.target_type}:${candidate.target_id}`}>
                  <div className="row" style={{ justifyContent: 'space-between' }}>
                    <div>
                      <span className={`badge ${candidate.blocked_title_pattern_evidence?.length ? 'err' : candidate.allowed_title_pattern_evidence?.length ? 'ok' : ''}`}>
                        {Math.round((candidate.confidence_score || 0) * 100)}%
                      </span>{' '}
                      <strong>{candidate.canonical_product_name}</strong>
                      <span className="muted"> · {candidate.target_type}:{candidate.target_id || '-'}</span>
                    </div>
                    <div className="row">
                      <button onClick={() => runAction(card, 'select_existing_candidate', candidate)}>이 후보 선택</button>
                      <button onClick={() => runAction(card, 'add_allowed_title_pattern', candidate)}>허용 제목 추가</button>
                      <button className="danger-outline" onClick={() => runAction(card, 'add_blocked_title_pattern', candidate)}>차단 제목 추가</button>
                    </div>
                  </div>
              ...
              {(!card.candidates || card.candidates.length === 0) && (
                <div className="muted">기존 후보가 없습니다. variant/source listing 생성 또는 수동 보류를 선택하세요.</div>
              )}
            </div>
            <details className="inline-details" style={{ marginTop: 8 }}>
              <summary>고급: 원본/후보 JSON</summary>
              <pre className="json-block">{JSON.stringify(card, null, 2)}</pre>
            </details>
          </div>
        ))}
      </div>
      {cards.length > 20 && <div className="muted">처음 20개 카드만 표시합니다.</div>}
    </details>
  );
}
...
export default function ReviewQueuePanel() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const [proposals, keywordProposalResponse, matchCardResponse, records, auditReport, publishEligibility, operatorResult, setupResult] = await Promise.all([
        requestJson('/api/review/proposals'),
        requestJson('/api/review/keyword-proposals'),
        requestJson('/api/review/match-cards'),
        ...
        requestJson('/api/review/operator-dashboard-summary')
      ...
      if (operatorResult.ok) {
        ...
        setOperatorSummaryError(operatorResult.error.message);
      }
      if (setupResult.ok) {
  ...
  const dashboardPublishSummary = useMemo(() => (
    operatorSummary?.stats
      ? {
        ...(publishSummary || {}),
        ...operatorSummary.stats,
        blockers: operatorSummary.blocker_buckets || publishSummary?.blockers || [],
      }
      : publishSummary
  ), [operatorSummary, publishSummary]);
  const healthCards = useMemo(() => (
    buildBatchHealth(dashboardPublishSummary || {}, rawRecords, items, keywordProposals)
  ), [dashboardPublishSummary, rawRecords, items, keywordProposals]);
  const triageCounters = useMemo(() => (
    buildOpsTriageCounters(dashboardPublishSummary || {}, publishRows)
  ), [dashboardPublishSummary, publishRows]);
    ...
    try {
      const result = await requestJson('/api/review/publish-approved', {
        ...
        body: {
          ...
          ...(batchIds.length === 1 ? { batch_id: batchIds[0] } : {}),
        },
      });
      setError(`DB-admin 제출 완료: 최종 승인 ${result.ai_safe_final_approved ?? 0}, 검수 대기 ${result.pending_db_review ?? 0}, 최종 승인 실패 ${result.final_approve_failed ?? 0}, 제출 실패 ${result.failed}`);
      await refresh();
    } catch (err) {
  ...
  return (
    <section id="review" className="panel review-panel anchor-offset">
      <div className="row" style={{ justifyContent: 'space-between' }}>
        <h2>AI 검수 대시보드 <span className="muted">({items.length}개 제안 / {keywordProposals.length}개 키워드 제안 / {matchCards.length}개 매치 카드 / {rawRecords.length}개 원본)</span></h2>
      ...
      />
      <OperatorDashboardSummary operatorSummary={operatorSummary} operatorSummaryError={operatorSummaryError} />
      <ProviderStatusStrip providerSummary={providerSummary} setupError={providerSetupError} />
      <PipelineStatusBar rawRecords={rawRecords} items={items} audit={audit} publishSummary={dashboardPublishSummary} />
      <WorkflowGuide audit={audit} rawCount={rawRecords.length} proposalCount={items.length} visibleGroups={filteredGroups.length} publishSummary={dashboardPublishSummary} />
      <PublishControls
