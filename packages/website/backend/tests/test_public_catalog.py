...
class PriceObservationCatalogStorage:
    def get_product_detail(self, product_id):
        if product_id != 4:
            return None
        return {
            "product": {
                "canonical_name": "두부 300g",
            ...
            "offer": {
                "source_name": "emart",
                "source_title": "국산콩 두부 300g",
                "source_url": "https://example.test/tofu",
                "image_url": "https://example.test/tofu.jpg",
                "price": 1980,
                "raw_data": {
...
def test_product_endpoint_exposes_price_observation_without_fake_discount():
    client = TestClient(create_app(storage=PriceObservationCatalogStorage()))

    resp = client.get("/api/products/4")

    assert resp.status_code == 200
    data = resp.json()["data"]
    assert data["name"] == "두부 300g"
    assert data["price"] == 1980
    assert data.get("original_price") is None
    assert data["price_observation_only"] is True
    assert data["has_discount_metadata"] is False
    assert data["record_label"] == "관측 가격"
    assert data["discount_claim_status"] == "unknown"


def test_price_history_exposes_observation_metadata_without_discount_claim():
    client = TestClient(create_app(storage=PriceObservationCatalogStorage()))

    resp = client.get("/api/products/4/price-history?days=30")

    assert resp.status_code == 200
    data = resp.json()["data"]
    assert data["point_count"] == 1
    point = data["history"][0]
    assert point["price"] == 1980
    assert point["price_observation_only"] is True
    assert point["has_discount_metadata"] is False
    assert point["original_price"] is None
    assert point["discount_rate"] is None
    assert point["record_label"] == "관측 가격"
    assert point["source_url"] == "https://example.test/tofu"
