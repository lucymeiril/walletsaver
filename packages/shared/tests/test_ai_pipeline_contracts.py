...
def test_product_offer_draft_preserves_customer_safe_offer_variant_and_audit_fields():
    ...
    assert item["price_state"] == "normal"
    assert item["promotion_type"] == "was_now_price"
    assert item["package_quantity"] == 300
    assert item["price_per_100g"] == 4950
    assert item["raw_data"]["raw_evidence"]["raw_unit"] == "100g"


def test_ai_offer_draft_allows_discount_rate_only_without_fake_price():
    draft = SaleOfferDraft(
        source_name="emart",
        source_title="카드 20% 할인",
        price_state="discount_rate_only",
        promotion_type=PromotionType.CHECKOUT_DISCOUNT,
        discount_rate=0.2,
        raw_record_id="raw-card-20",
        raw_evidence={"text": "카드 결제 시 20% 할인"},
    )

    assert draft.price is None
    assert draft.original_price is None
    assert draft.discount_rate == pytest.approx(0.2)
