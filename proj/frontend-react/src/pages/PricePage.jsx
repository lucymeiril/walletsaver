...
import { PRODUCTS, MARTS, MART_DATA, HOTDEALS, fmt, genPriceHistory } from '../data/mockData';
...
export default function PricePage() {
  ...
  const diff = product.cur - product.avg;

  let timing = {};
  if (ratio <= 0.7)       timing = { cls: 'ultra', icon: '🔥', title: '역대급 기회!',       desc: `현재 ${fmt(product.cur)}원은 평균보다 ${Math.round((1-ratio)*100)}% 저렴합니다. 지금 바로 구매하세요!` };
  else if (ratio <= 0.85) timing = { cls: 'great', icon: '💙', title: '좋은 가격이에요!',   desc: `현재 ${fmt(product.cur)}원은 평균(${fmt(product.avg)}원)보다 ${Math.round((1-ratio)*100)}% 저렴합니다.` };
  else if (ratio <= 1.05) timing = { cls: 'good',  icon: '✅', title: '지금 사도 괜찮아요!', desc: `현재 ${fmt(product.cur)}원은 평균(${fmt(product.avg)}원) 수준입니다. (${diff >= 0 ? '+' : ''}${fmt(diff)}원)` };
  else                    timing = { cls: 'wait',  icon: '⏳', title: '조금 기다려보세요',   desc: `현재 ${fmt(product.cur)}원은 평균보다 ${Math.round((ratio-1)*100)}% 비쌉니다.` };

  const tierPos = Math.max(3, Math.min(97, ((product.cur - product.low) / (product.high - product.low)) * 100));
      ...
      <div className={s.layout}>
        <div className={s.left}>
          ...
          <div className={s.prices}>
            <div className={`${s.priceBox} ${s.current}`}><span className={s.label}>현재 평균</span><span className={s.val}>{fmt(product.cur)}원</span></div>
            <div className={s.priceBox}><span className={s.label}>30일 평균</span><span className={s.val}>{fmt(product.avg)}원</span></div>
            <div className={`${s.priceBox} ${s.low}`}><span className={s.label}>최근 최저</span><span className={s.val}>{fmt(product.low)}원</span></div>
            <div className={`${s.priceBox} ${s.high}`}><span className={s.label}>최근 최고</span><span className={s.val}>{fmt(product.high)}원</span></div>
          </div>
